export const metadata = {
    "home": {
        "title": "Google Developer Groups Taiwan",
        "description": "Google Developer Groups(GDG)는 개발자가 다른 사람들과 연결하고 모든 Google 플랫폼에서 성공적으로 구축하는 데 가장 도움이 되는 방법이 되는 것을 목표로 합니다. 전 세계 145개국 이상에 1,000개 이상의 GDG 커뮤니티가 있습니다. Google Developer Groups는 세계에서 가장 큰 개발자 커뮤니티 네트워크입니다. 각 GDG는 Google 개발자 기술에 대한 전문 지식과 열정을 공유하는 전문 개발자들의 지역 커뮤니티 허브입니다. GDG 프로그램의 초점은 개발자가 Google의 최신이자 최고의 개발자 기술에 접근하고, 함께 배우고, 성공적인 제품을 함께 구축하며, 기술 업계의 다른 개발자 및 전문가와 연결할 수 있도록 돕는 것입니다."
    },
    "activity": {
        "title": "최근 활동 | Google Developer Groups Taiwan",
        "description": "Google Developer Groups Taiwan의 최근 활동"
    },
    "annual_activity": {
        "international_womens_day": {
            "title": "국제 여성의 날 | Google Developer Groups Taiwan",
            "description": "국제 여성의 날(IWD)은 Women Techmakers의 가장 큰 연례 행사입니다. 앰배서더들은 3월과 4월에 전 세계에서 행사를 개최하여 이 순간을 기념합니다. 대규모 서밋부터 소규모 프라이빗 모임까지, IWD는 모든 여성이 기술 분야에서 번영할 수 있는 세상을 만드는 우리의 미션을 지원하는 방법이기도 합니다."
        },
        "google_io_extended": {
            "title": "Google I/O 확장 행사 | Google Developer Groups Taiwan",
            "description": "Google I/O는 전 세계의 개발자들이 모여 심도 있는 토론을 하고, Google 전문가와 함께 실습 학습을 하며, Google의 최신 개발자 제품을 가장 먼저 체험할 수 있는 행사입니다. 메인 이벤트가 끝난 후, 현지 커뮤니티는 I/O Extended 행사를 통해 기술을 공유하고, Q&A를 진행하며, 다른 개발자들과 교류를 이어갑니다."
        },
        "cloud_study_jam": {
            "title": "클라우드 스터디 잼 | Google Developer Groups Taiwan",
            "description": "컨테이너화된 애플리케이션부터 가상 머신 생성까지, Study Jam은 주제와 기술 수준에 따라 맞춤화할 수 있습니다. 클라우드 기초 입문부터 머신러닝, BigQuery, 인증, Kubernetes의 심화 학습까지, Google Cloud의 핵심 도구와 기능을 무료 실습 랩을 통해 배울 수 있습니다."
        },
        "devfest": {
            "title": "DevFest | Google Developer Groups Taiwan",
            "description": "DevFest는 전 세계 Google Developer Groups(GDG)가 주최하는 지역 기술 컨퍼런스입니다. 현지 주최자는 커뮤니티의 요구에 따라 행사를 기획하며, 실습 경험부터 전문가 기술 강연까지, Google 오픈소스 기술과 개발 도구를 사용해 참가자들이 협력하고 혁신할 수 있도록 이끕니다."
        }
    },
    "chapters": {
        "title": "대만의 모든 GDG 지부 | Google Developer Groups Taiwan",
        "description": "대만의 모든 GDG 지부"
    },
    "notFound":{
        "title": "404 - 페이지를 찾을 수 없습니다",
        "description": "죄송합니다. 찾고 계신 페이지가 존재하지 않습니다."
    }
};